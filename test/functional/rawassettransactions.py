...
class RawAssetTransactionsTest(SucrecoinTestFramework):
    ...
    def issue_reissue_transfer_test(self):
        ...
        for i in range(1, 3):
            ...
            f = BytesIO(hex_str_to_bytes(script_hex[script_hex.index(xsrt) + len(xsrt):-len(op_drop)]))
            ...
            tampered_script = script_hex[:script_hex.index(xsrt)] + xsrt + tampered_transfer + op_drop
