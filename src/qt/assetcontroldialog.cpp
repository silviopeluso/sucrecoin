...
void AssetControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    l2->setText(SucrecoinUnits::formatWithCustomName(QString::fromStdString(strAssetName), nAssetAmount));        // Amount
    ...
    l8->setText(SucrecoinUnits::formatWithCustomName(QString::fromStdString(strAssetName), nChange));        // Change
    if (nPayFee > 0)
    {
...
void AssetControlDialog::updateView()
{
    ...
    for (const std::pair<QString, std::vector<COutput>> &coins : mapAssetCoins) {
        ...
        for (const COutput &out : coins.second) {
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, SucrecoinUnits::format(nDisplayUnit, nAmount));
