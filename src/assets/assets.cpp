...
static const std::regex SUCRECOIN_NAMES("^XSR$|^SUCRECOIN$|^SUCRECOIN$");
...
void CNewAsset::ConstructTransaction(CScript& script) const
{
    ...
    script << OP_XSR_ASSET << ToByteVector(vchMessage) << OP_DROP;
}
...
{
    // Issuing an Asset must contain at least 3 CTxOut( Sucrecoin Burn Tx, Any Number of other Outputs ..., Owner Asset Tx, New Asset Tx)
    if (vout.size() < 3) {
...
{
    // Must contain at least 3 outpoints (XSR burn, owner change and one or more new unique assets that share a root (should be in trailing position))
    if (vout.size() < 3) {
...
{
    // Verify 'xsrq' is in the transaction
...
{
    // Verify 'xsrr' is in the transaction
    CScript scriptPubKey = txOut.scriptPubKey;
